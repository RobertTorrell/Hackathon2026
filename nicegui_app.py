...
async def handle_upload(event: Any) -> None:
    ...
    raw_name = getattr(event, "name", None) or getattr(uploaded_file, "name", None) or "uploaded_workbook.xlsx"
    ...
    content = getattr(event, "content", None) or uploaded_file
    if content is None:
        raise ValueError("NiceGUI upload event did not include file content.")
    ...
    if hasattr(content, "read"):
        ...
        raise ValueError(f"Unsupported upload content type: {type(content).__name__}")
