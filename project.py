...
DEFAULT_MODEL_CONFIG = {
    ...
    "type_order_grid": [(0, 1, 1), (1, 1, 0), (1, 1, 1), (2, 1, 1)],
    "type_seasonal_order_grid": [(0, 0, 0, 0), (1, 0, 0, 52)],
    "product_order": (1, 0, 1),
    "product_seasonal_order": (0, 0, 0, 0),
    "product_order_grid": [(0, 0, 1), (1, 0, 0), (1, 0, 1), (2, 0, 1)],
    "product_seasonal_order_grid": [(0, 0, 0, 0)],
    "min_model_weeks": 36,
...
DEFAULT_CLIENT_WINDOW_CONFIG = {
    "active_client_lookback_weeks": 52,
    "product_mix_lookback_weeks": 52,
    "potential_capture_weeks": 13,
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--client-window-config-json",
        default=None,
        help="Optional JSON file overriding active-client, product-mix, and potential-capture windows.",
    )
...
def load_model_config(path: str | None = None) -> dict[str, Any]:
    ...
    for key in ["type_order", "product_order"]:
        config[key] = tuple(config[key])
    for key in ["type_seasonal_order", "product_seasonal_order"]:
        config[key] = tuple(config[key])
    for key in ["type_order_grid", "product_order_grid"]:
        config[key] = [tuple(value) for value in config[key]]
    for key in ["type_seasonal_order_grid", "product_seasonal_order_grid"]:
        config[key] = [tuple(value) for value in config[key]]
    return config


def load_client_window_config(path: str | None = None) -> dict[str, int]:
    config = DEFAULT_CLIENT_WINDOW_CONFIG.copy()
    if path:
    ...
    for order in order_grid:
        for seasonal_order in seasonal_grid:
            try:
                ...
                score = float(getattr(fitted, metric, fitted.aic))
                ...
                continue
    if best_fit is None:
        raise ValueError("No SARIMA/SARIMAX candidate could be fitted")
...
def model_forecast(
    ...
    try:
        if product_level:
            order_grid = model_config["product_order_grid"] if model_config.get("auto_tune", True) else [model_config["product_order"]]
            seasonal_grid = model_config["product_seasonal_order_grid"] if model_config.get("auto_tune", True) else [model_config["product_seasonal_order"]]
        else:
            order_grid = model_config["type_order_grid"] if model_config.get("auto_tune", True) else [model_config["type_order"]]
            seasonal_grid = model_config["type_seasonal_order_grid"] if model_config.get("auto_tune", True) else [model_config["type_seasonal_order"]]
        if len(y) < int(model_config["min_seasonal_weeks"]):
            seasonal_grid = [(0, 0, 0, 0)]
        metric = str(model_config.get("selection_metric", "aic")).lower()
        if metric not in {"aic", "bic"}:
            metric = "aic"
        fitted, order, seasonal_order = fit_best_sarimax(y, order_grid, seasonal_grid, maxiter, metric, exog=exog)
        ...
        return ForecastResult(predicted, method, order=order, seasonal_order=seasonal_order, aic=float(fitted.aic), bic=float(fitted.bic))
        ...
        part = pd.DataFrame(
            {
                ...
                "selected_order": str(result.order) if result.order is not None else None,
                "selected_seasonal_order": str(result.seasonal_order) if result.seasonal_order is not None else None,
...
def build_client_features(
    ...
    client_window_config: dict[str, int] | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    classification_config = classification_config or DEFAULT_CLASSIFICATION_CONFIG
    client_window_config = client_window_config or DEFAULT_CLIENT_WINDOW_CONFIG
    ...
    active_weeks_window = pd.DatetimeIndex(history_weeks[-client_window_config["active_client_lookback_weeks"]:])
    capture_weeks_window = pd.DatetimeIndex(history_weeks[-client_window_config["potential_capture_weeks"]:])
    product_mix_weeks = pd.DatetimeIndex(history_weeks[-client_window_config["product_mix_lookback_weeks"]:])
    ...
    spend_active_window = sum_window(cat_sales, active_weeks_window, ["client_id", "category_h", "analytical_block"]).rename(columns={"revenue": "spend_active_window"})
    spend_capture_window = sum_window(cat_sales, capture_weeks_window, ["client_id", "category_h", "analytical_block"]).rename(columns={"revenue": "spend_capture_window"})
    ...
    active_52 = (
        cat_sales[cat_sales["week_start"].isin(active_weeks_window) & (cat_sales["revenue"] > 0)]
    ...
    features = features.merge(spend_active_window, on=["client_id", "category_h", "analytical_block"], how="left")
    features = features.merge(spend_capture_window, on=["client_id", "category_h", "analytical_block"], how="left")
    ...
    features["potential_capture_window"] = features["annual_potential"] * client_window_config["potential_capture_weeks"] / 52.0
    ...
    features["capture_ratio_window"] = features["spend_capture_window"] / features["potential_capture_window"].replace(0, np.nan)
    features["capture_ratio_window"] = features["capture_ratio_window"].replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(lower=0.0, upper=3.0)
    ...
    features["classification_eligible"] = features["active_weeks_52w"] > 0
    ...
    features, product_driver = add_future_projection(features, weekly_sales, product_forecasts, product_mix_weeks)
    ...
    replaced_capture_spend = (features["spend_capture_window"] - features["spend_4w"] + features["predicted_spend_4w"]).clip(lower=0.0)
    features["predicted_capture_ratio_window"] = replaced_capture_spend / features["potential_capture_window"].replace(0, np.nan)
    features["predicted_capture_ratio_window"] = features["predicted_capture_ratio_window"].replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(lower=0.0, upper=3.0)
    ...
    features["future_state"] = classify_state(
        ...
        features["predicted_capture_ratio_window"],
...
def add_future_projection(
    ...
    weekly_sales: pd.DataFrame,
    product_forecasts: pd.DataFrame,
    product_mix_weeks: pd.DatetimeIndex,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    product_summary = (
    ...
    client_product = (
        weekly_sales[weekly_sales["week_start"].isin(product_mix_weeks)]
        .groupby(["client_id", "category_h", "analytical_block", "product_id", "family_h"], as_index=False)["revenue"]
        .sum()
        .rename(columns={"revenue": "client_product_spend_mix_window"})
    )
    client_product_4 = (
        weekly_sales[weekly_sales["week_start"].isin(product_mix_weeks[-4:])]
        .groupby(["client_id", "category_h", "analytical_block", "product_id", "family_h"], as_index=False)["revenue"]
        .sum()
    ...
    weighted["weighted_factor"] = weighted["client_product_spend_mix_window"].clip(lower=0) * weighted["product_forecast_factor"]
    factor = (
        weighted.groupby(["client_id", "category_h", "analytical_block"], as_index=False)
        .agg(weighted_factor=("weighted_factor", "sum"), positive_spend_mix_window=("client_product_spend_mix_window", lambda s: s.clip(lower=0).sum()))
    )
    factor["client_product_forecast_factor"] = (
        factor["weighted_factor"] / factor["positive_spend_mix_window"].replace(0, np.nan)
    ).replace([np.inf, -np.inf], np.nan).fillna(1.0).clip(lower=0.2, upper=2.5)

    slope_weighted = client_product.copy()
    slope_weighted["product_importance_weight"] = slope_weighted["client_product_spend_mix_window"].clip(lower=0)
    ...
    inactive_with_annual_history = (base_spend <= 0) & (out["prev_spend_4w"] <= 0) & (out["spend_active_window"] > 0)
    base_spend = base_spend.mask(inactive_with_recent_history, out["prev_spend_4w"] * 0.65)
    base_spend = base_spend.mask(inactive_with_annual_history, (out["spend_active_window"] / 13.0) * 0.35)
...
def main() -> None:
    ...
    client_window_config = load_client_window_config(args.client_window_config_json)
